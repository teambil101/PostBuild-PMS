...
/**
 * Pipeline view — extracted from the legacy Leads page so it can render
 * inside the People page as a tab. Behaviorally identical: same URL params
 * (q, status, assignee, source, closeFrom, closeTo, stuck), same KPIs,
 * same kanban/table toggle.
 *
 * `headerAction` lets the host page own the "+ New lead" button placement
 * (we render it in the page header, not above the KPI strip).
 */
export function PipelineView({ headerAction }: { headerAction?: React.ReactNode } = {}) {
  ...
  const update = (k: string, v: string | null) => {
    ...
    // Preserve the tab=pipeline param when present.
  ...
  const load = async () => {
    ...
    const contractIds = list.map((l) => l.won_contract_id).filter((x): x is string => !!x);
