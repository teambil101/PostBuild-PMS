-- Yahya AI assistant: chat tables + read-only SQL runner
...
-- 3) Read-only SQL runner
-- Security model:
--   * SECURITY DEFINER, owned by postgres, runs with full read access.
--   * Caller must be an authenticated user with role 'admin' or 'staff'.
--   * Query must be a single SELECT or WITH ... SELECT.
--   * Forbidden keywords (DDL/DML/auth schemas) blocked by regex.
--   * statement_timeout set to 8 seconds.
--   * Wrapped in a read-only transaction.
--   * Hard cap of 500 rows returned as jsonb.
...
begin
  ...
  -- No additional semicolons (no statement chaining)
  ...
  if q_lower ~ '\b(insert|update|delete|drop|alter|truncate|create|grant|revoke|comment|copy|vacuum|analyze|reindex|cluster|listen|notify|do|call|execute|prepare|deallocate|reset|set\s+session|set\s+role|lock|refresh)\b' then
    raise exception 'forbidden_keyword';
  end if;

  -- Block sensitive schemas / functions
  if q_lower ~ '\b(pg_catalog|pg_authid|pg_shadow|pg_user|pg_roles|information_schema\.role|auth\.|storage\.|vault\.|supabase_functions\.|net\.|extensions\.)' then
    raise exception 'forbidden_schema';
  end if;

  if q_lower ~ '\b(pg_read_server_files|pg_write_server_files|pg_ls_dir|pg_read_file|dblink|copy\s+from|copy\s+to)\b' then
    raise exception 'forbidden_function';
  end if;

  -- Run inside a read-only transaction with a timeout
